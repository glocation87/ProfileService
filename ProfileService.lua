...
if (not game:GetService("RunService"):IsServer()) then return error "Util [ProfileService] can only be accessed from the Server" end
...
local function SaveProfileAsync(profile, release_from_session, is_overwriting)
	local dataProxy = getmetatable(profile.Data).__index
	if (dataProxy and type(dataProxy) ~= "table") or type(profile.Data) ~= "table" then
		RegisterCorruption(
...
local Profile = {
	--[[
		...
		_data_change_listeners = [ScriptSignal] / nil, -- [table / nil]
			...
			if type(active_session) == "table" then
				if IsThisSession(active_session) == true then
					...
					local profile = {
						...
						_data_change_listeners = Madwork.NewScriptSignal(),
						_hop_ready = false,
